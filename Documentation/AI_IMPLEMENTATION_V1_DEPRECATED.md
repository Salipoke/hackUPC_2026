# BioMeshP2P - EdgeAI Implementation Guide (V1, DEPRECATED)

> **DEPRECATED.** This document was the original plan based on Edge Impulse + `@tensorflow/tfjs-tflite-node` (TFLite native bindings). It is kept for historical reference only.
>
> Reasons for deprecation:
> 1. Edge Impulse Studio UI no longer exposes the raw `.tflite` export in the same place — workflow is broken for our needs.
> 2. `@tensorflow/tfjs-tflite-node` requires native compilation (libtensorflow), is incompatible with newer Node versions, and conflicts with the Pear/Bare runtime sandbox.
> 3. We confirmed via spike (see `scripts/spike-pear-test.js`) that pure `@tensorflow/tfjs` works under Pear with a tiny shim. Training in Node.js with the same library is fast enough for our model size.
>
> **The current plan is `AI_IMPLEMENTATION.md` (V2).** Read that one.
